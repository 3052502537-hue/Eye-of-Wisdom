 ...
 *           24GHz FMCW, UART 256000bps, 检测运动目标(0.5-8m, ±60°), 最多3目标
 *           数据帧: AA FF 03 00 + 目标1(8B)+目标2(8B)+目标3(8B) + 55 CC = 30字节
 *           坐标编码: 符号-幅度+反相符号位(MSB=1→正), 非标准补码
 *           速度单位: cm/s (原始) → m/s (输出)
 *           本驱动用于前方/后方两路雷达（实例化两个对象）
 * 依赖关系: Arduino Serial(UART)、config.h、protocol.h
 * 接口说明:
 *   RD03DDriver()        - 构造函数
 *   begin()              - 初始化指定UART(256000bps 8N1)
